...
int* find(int* a, int n, bool func(int)) {
	for (int i = n-1; i >= 0; i--) {
		if (func(*(a+i)))
			return (a + i);
	}
...
//Yeu cau 4
int findExtrem(int* a, int n, int j = 0, int pos = -1) {
	if (j == n)
		return pos;
...
void sortArr(int* a, int n, int i=0) {
	...
	//Hoan vi voi phan tu dau day dang xet
	swap(a[i], a[pos]);
...
int main() {
	...
	cout << "==============DEM SO CHAN============\n";
	cout << "Ket qua dem: " << count(a, n) << endl;
